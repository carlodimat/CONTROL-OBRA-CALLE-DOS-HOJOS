...
if df is not None:
    ...
    m1.metric("TOTAL ING", f"$ {total_ing:,.2f}")
    m2.metric("NETO", f"$ {total_neto:,.2f}")
    ...
    with t1:
        def update_mobile_style(fig, max_val):
            fig.update_layout(
                ...
                font=dict(color="#000000", size=11), # Letra más pequeña para los ejes
                yaxis=dict(tickfont=dict(size=11, color="#000000")),
                # Rango extendido al 50% extra para que quepan los montos
                xaxis=dict(showticklabels=False, range=[0, max_val * 1.5]),
                margin=dict(l=10, r=180, t=30, b=10) # Margen derecho extremo para seguridad
            )
            fig.update_traces(textposition='outside', textfont=dict(color="black", size=12, family="Arial Black"))
            return fig

        # Nombre con salto de línea para ahorrar espacio
        label_admin = "ADMINISTRACIÓN<br>DELEGADA"

        # --- CATEGORÍAS ---
        st.write("### 📌 Categorías + Admin")
        df_t = df_gastos.groupby('TIPO')['MONTO BASE USD'].sum().reset_index()
        df_t = pd.concat([df_t, pd.DataFrame({'TIPO':[label_admin], 'MONTO BASE USD':[total_honorarios]})], ignore_index=True).sort_values('MONTO BASE USD', ascending=True)
        ...
        df_a = pd.concat([df_a, pd.DataFrame({'AREA':[label_admin], 'MONTO BASE USD':[total_honorarios]})], ignore_index=True).sort_values('MONTO BASE USD', ascending=True)
        ...
        df_p = pd.concat([df_p, pd.DataFrame({'PROVEEDOR':[f'ADMIN'], 'MONTO BASE USD':[total_honorarios]})], ignore_index=True).sort_values('MONTO BASE USD', ascending=True)
